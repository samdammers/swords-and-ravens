...
export default class ResolveMoveLoyaltyTokenGameState extends GameState<MoveLoyaltyTokensGameState> {
  ...
  onPlayerMessage(player: Player, message: ClientMessage): void {
    if (message.type == "move-loyalty-token") {
      ...
      if (this.moveLoyaltyTokensState.acceptAllMovements) {
        ...
        this.moveLoyaltyTokensState.setChooseCancelLastMoveGameState(
          this.house
        );
      }
    }
  ...
  getRequiredVisibleRegionsForPlayer(_player: Player): Region[] {
    ...
    const adjacentRegions = _.flatMap(
      regionsWithLT.map((r) =>
        this.ingame.world
          .getNeighbouringRegions(r)
          .filter((r2) => r2.type == land)
      )
    );
  ...
  static deserializeFromServer(
    moveLoyaltyTokens: MoveLoyaltyTokensGameState,
    data: SerializedResolveMoveLoyaltyTokenGameState
  ): ResolveMoveLoyaltyTokenGameState {
